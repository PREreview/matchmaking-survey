...
const scientist = (overrides: Partial<{ orcid: string; token: string; submitted_at: string | null }> = {}) => ({
...
const batch = (overrides: Partial<{ id: number; uploaded_at: string; scientists: ReturnType<typeof scientist>[] }> = {}) => ({
  id: 1,
...
describe("renderAdminPage", () => {
  ...
  it("renders no error summary when there is no duplicate error", () => {
    const result = renderAdminPage(base).__html
    expect(result).not.toContain('<div class="error-summary"')
  })
  ...
  it("renders an error summary listing duplicate orcid/doi pairs", () => {
    ...
    expect(result).toContain('<div class="error-summary"')
  ...
  it("shows a success banner for the highlighted batch", () => {
    const result = renderAdminPage({
      ...base,
      batches: [batch({ id: 7, scientists: [scientist(), scientist({ orcid: "0000-0002-2222-2222" })] })],
      highlightBatchId: 7,
    }).__html
      ...
      batches: [
        batch({
          ...
          scientists: [
            ...
            scientist({ orcid: "0000-0002-2222-2222", token: "tok-b", submitted_at: "2026-07-01 00:00:00" }),
          ],
        }),
      ],
    }).__html
  ...
  it("renders a copy-links textarea with tab-separated orcid and full url per line", () => {
    const result = renderAdminPage({
      ...base,
      batches: [batch({ id: 3, scientists: [scientist({ orcid: "0000-0001-1111-1111", token: "tok-a" })] })],
    }).__html
