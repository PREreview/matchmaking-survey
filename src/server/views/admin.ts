...
function renderBatch(
  ...
  const links = batch.scientists
    .map((s) => `${s.orcid}\t${origin}/s/${s.token}`)
    .join("\n")
  ...
  >
    ...
    <div style="margin-top:0.75rem;">
      ${isHighlighted
        ? html`<p style="color:#198754;">Batch #${batch.id} created — ${batch.scientists.length} scientists</p>`
        : raw("")}
      <label for="links-${batch.id}">Survey links for this batch (orcid, then url, one per line)</label>
      <br />
