...
describe("importCsv", () => {
  ...
  it("assigns correct number of papers to each scientist", async () => {
    const result = await run(Admin.importCsv(csvText))
    const orcid1Entry = result.entries.find(
      (e) => e.orcid === "0000-0001-1111-1111",
    )
    expect(orcid1Entry?.paperCount).toBe(2)
    const orcid2Entry = result.entries.find(
      (e) => e.orcid === "0000-0002-2222-2222",
    )
        ...
        Effect.andThen((scientist) =>
          Db.listPapersForScientist(scientist!.id).pipe(
            Effect.andThen((papers) =>
              Db.upsertResponse(scientist!.id, papers[0].id, 5),
            ),
          ),
        ),
