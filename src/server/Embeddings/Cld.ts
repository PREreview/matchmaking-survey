...
import { UnableToDetectLanguage } from "./Shared";
...
export const detectLanguage = Effect.fn("LanguageDetection.detectLanguage")(
  function* (
    ...
    const isHTML = typeof text !== "string";
    const plainText = typeof text === "string" ? text : text.__html;

    const result = yield* Effect.option(
      Effect.orElse(
        Effect.tryPromise(() => cld.detect(plainText, { bestEffort: true, isHTML, languageHint })),
        () => Effect.tryPromise(() => cld.detect(plainText, { bestEffort: true, isHTML })),
      ),
    );

    return Option.andThen(result, (result) =>
      Array.findFirst(result.languages, (detected) =>
        Option.liftPredicate(detected.code, iso6391.validate),
      ),
    );
  },
  Effect.andThen(
    Option.match({ onSome: Effect.succeed, onNone: () => new UnableToDetectLanguage({}) }),
  ),
);
